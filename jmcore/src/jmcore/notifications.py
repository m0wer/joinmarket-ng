...
def load_notification_config() -> NotificationConfig:
    ...
    return NotificationConfig(
        enabled=enabled,
        urls=urls,
        title_prefix=os.environ.get("NOTIFY_TITLE_PREFIX", "JoinMarket"),
...
class Notifier:
        ...
        async with self._lock:
            ...
            try:
                ...
                if self.config.use_tor:
                    ...
                    # Set environment variables that Apprise will use
    ...
    async def _send(
        ...
        try:
            ...
            if hasattr(self._apprise, "async_notify"):
            ...
            if not result:
                logger.debug(f"Notification may have failed: {title}")
            return result
    ...
    def _format_nick(self, nick: str) -> str:
        """Format nick for display (truncated for privacy)."""
        if not self.config.include_nick:
            return "[hidden]"
        return f"{nick[:8]}..." if len(nick) > 8 else nick
